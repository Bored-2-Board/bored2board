...
import {
  addWishlistGame,
  removeWishlistGame,
} from "../src/app/store/WishlistGames/wishlistGamesSlice";
...
export default function GameCard({
  ...
  const removeFromWishList = async () => {
    ...
    await fetch(
      `/api/wishlist/delete?name=${name}&userID=${userData}`,
      settings
    );
  };
    ...
    <div className="card flex min-w-[300px] max-w-[10%] min-h-[435px] max-h-[435px] g-base-100 shadow-xl m-3">
      <div className="flex justify-center">
        <img
          ...
          height={200}
          id="gamecard-image"
          className="rounded-2xl w-[95%] max-h-[200px] mt-2"
        />
      </div>
      <div className="card-body flex-col max-w-full overflow-hidden">
        ...
        <p className="text-slate-600 font-bold">
          Game Length: {gameLength} min
        </p>
        <p className="text-slate-600 font-bold">Price: {price}</p>
        <a
          href={link ? link : `https://www.google.com/search?q=${name}`}
          target="_blank"
          className="underline text-slate-500 font-semibold"
        >
          Check it out here!
        </a>
      </div>
      <div className="flex justify-end rating gap-1 px-2 mb-2">
        <input
          ...
          name="rating-1"
          className={`${
            wish ? "mask mask-heart bg-red-400" : "mask mask-heart bg-gray-400"
          }`}
