...
export async function GET(req: Request) {
  try {
    ...
    const year = `gt_year_published=${currentYear - 1}&lt_year_published=${
      currentYear + 1
    }`;

    // FETCH
    const response = await fetch(
      `https://api.boardgameatlas.com/api/search?${order}&${fields}&${year}&client_id=JLBr5npPhV`
    );
    ...
    return NextResponse.json({
      message: "Error with GET request for New Games",
      error,
      status: 500,
    });
  }
